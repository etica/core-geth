...
func ethFilter(args []string) (nodeFilter, error) {
	...
	case "etica":
		gb := core.GenesisToBlock(params.DefaultEticaGenesisBlock(), nil)
		filter = forkid.NewStaticFilter(params.EticaChainConfig, gb)
	case "crucible":
		gb := core.GenesisToBlock(params.DefaultCrucibleGenesisBlock(), nil)
		filter = forkid.NewStaticFilter(params.CrucibleChainConfig, gb)
	case "mintme":
